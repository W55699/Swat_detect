...
# Compute accuracy
acc_lgb = accuracy_score(y_test, np.argmax(y_pred_proba_lgb, axis=1))

# Compute recall
recall_lgb = recall_score(y_test, np.argmax(y_pred_proba_lgb, axis=1), average='weighted')

# Compute F1-score
f1_lgb = f1_score(y_test, np.argmax(y_pred_proba_lgb, axis=1), average='weighted')
...
# ROC Curve
n_classes = y_pred_proba_lgb.shape[1]
fpr = dict()
tpr = dict()
auc = dict()

for i in range(n_classes):
    fpr[i], tpr[i], _ = roc_curve((y_test == i).astype(int), y_pred_proba_lgb[:, i])
    auc[i] = roc_auc_score((y_test == i).astype(int), y_pred_proba_lgb[:, i])

# Compute micro-average ROC curve and ROC area
fpr["micro"], tpr["micro"], _ = roc_curve((y_test.ravel() == np.argmax(y_pred_proba_lgb, axis=1)).astype(int), 
                                          y_pred_proba_lgb[:, 1])
auc["micro"] = roc_auc_score((y_test.ravel() == np.argmax(y_pred_proba_lgb, axis=1)).astype(int), 
                             y_pred_proba_lgb[:, 1])
...
plt.title('ROC Curve for LightGBM Model (Multi-class)')
