...
# Compute accuracy
acc_xgb = accuracy_score(y_test, np.argmax(y_pred_proba_xgb, axis=1))

# Compute recall
recall_xgb = recall_score(y_test, np.argmax(y_pred_proba_xgb, axis=1), average='weighted')

# Compute F1-score
f1_xgb = f1_score(y_test, np.argmax(y_pred_proba_xgb, axis=1), average='weighted')
...
# Step 10: ROC Curve
n_classes = y_pred_proba_xgb.shape[1]
fpr = dict()
tpr = dict()
auc = dict()

for i in range(n_classes):
    fpr[i], tpr[i], _ = roc_curve((y_test == i).astype(int), y_pred_proba_xgb[:, i])
    auc[i] = roc_auc_score((y_test == i).astype(int), y_pred_proba_xgb[:, i])

# Compute micro-average ROC curve and ROC area
fpr["micro"], tpr["micro"], _ = roc_curve((y_test.ravel() == np.argmax(y_pred_proba_xgb, axis=1)).astype(int), 
                                          y_pred_proba_xgb[:, 1])
auc["micro"] = roc_auc_score((y_test.ravel() == np.argmax(y_pred_proba_xgb, axis=1)).astype(int), 
                             y_pred_proba_xgb[:, 1])
...
plt.title('ROC Curve for XGBoost Model (Multi-class)')
